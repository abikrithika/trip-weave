...
export async function signUp(req, res, next) {
  try {
    ...
    if (!signUpValidation.success) {
      return res.status(400).json({ success: false, errors: signUpValidation.error.flatten().fieldErrors });
    }
    ...
    const dkkCurrency = await prisma.currency.findUnique({ where: { code: 'DKK' } });
